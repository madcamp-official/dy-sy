...
## 2026-07-31 Potion-gated spell unlocks (hold left hand 3 s) + greyed-out HUD slots

- Asked for: hold the left hand on a potion to learn its spell; the spell is unusable until then; HUD shows it greyed until acquired. User picked **3 s hold** (down from their proposed 10 s — 10 s reads as broken in VR and eats a big slice of a 3–5 min demo) and **"lock the existing fireball"** rather than authoring a new shockwave spell.
- **The level contradicted the request and the user chose to change the level:** 1F (`z -403`, goblin floor) held `SM_Potion_Bottle_Green` and 2F (`z 203`, orc floor) held `SM_Potion_Bottle_Blue` — the opposite of "blue on 1F, green on 2F". Meshes were swapped on the two `StaticMeshActor`s so 1F is now blue (defense) and 2F green (attack), matching how the user describes it.
- What was already there and shaped the design:
  - `WBP_PlayerHUD` already has **three** slots (`MagicSlot1/2/3` × Frame/Icon/ChargeBar). Slot1 = fireball, Slot2 = defense, **Slot3 was hardcoded to `0.0` in `SetMagicCharges` and has no spell behind it** — still free for a future third spell.
  - `bFireballReady` / `bDefenseReady` are **cooldown** flags, not ownership. Added separate `bFireballUnlocked` / `bDefenseUnlocked` and ANDed them into the existing gates (`IfThenElse_9` for `IA_Grab_Left_Pressed`, `IfThenElse_11` for `IA_Menu_Toggle_Left`) rather than touching the cooldown logic.
- `BP_SpellPotion` (new, `/Game/Blueprints/Systems`): 0.1 s `PollHand` timer measures `Distance(BP_XRPawn.HandLeft world loc, self)` against `HandReachDistance` (45). In reach and no timer running -> `SetTimer(CompleteHold, HoldSeconds=3, non-looping)`; out of reach -> `ClearTimer`. `CompleteHold` sets `bCollected`, calls `UnlockSpell(bIsDefenseSpell)` on the pawn, plays sound + magic-circle burst, and hides the bottle via the instance-editable `PotionVisual` actor ref. **A timer is the whole progress model — no accumulate/clamp arithmetic**, which matters because promotable operators (`+`, `<=`, `Equal(Name)`) cannot be created through MCP. Comparisons use `InRange(Float)`; anything needing a ratio can use `MapRangeClamped(GetTimerRemainingTimeByFunctionName, HoldSeconds, 0, 0, 1)`.
- Hold feedback is a `P_ky_magicCircle1` emitter spawned attached on hold start, stored in `ChannelFX`, and `DestroyComponent`-ed on both cancel and completion. Verified by component list, not by the variable: after teardown `ChannelFX` still *reads* as a refPath (dangling pointer) while `get_components` correctly shows the emitter gone.
- HUD: new `WBP_PlayerHUD.SetMagicUnlocked(Slot1Unlocked, Slot2Unlocked)` tints Icon+Frame via `SelectColor`. **The widget's design-time defaults were also set to the locked greys**, because `GetUserWidgetObject` can return null at BeginPlay and a failed cast would otherwise leave the slots looking unlocked at t=0. `RefreshSkillCooldownHUD` feeds `1.0` through `SelectFloat` while locked so the charge bar reads empty instead of full-and-ready.
- Verified in PIE on `L_Dungeon`: both flags false at spawn; 3 s hold on the 1F blue potion set `bDefenseUnlocked` only; releasing after 1.5 s cancelled with nothing unlocked; the 2F green potion set `bFireballUnlocked`; bottles hid; `Accessed None|Blueprint Runtime Error|Broken Reference` returned **zero** entries across every run. All four assets compiled warnings-as-errors and confirmed on disk by mtime + `git status`.
- **Tooling traps hit this session:**
  - `BlueprintTools.create` with `/Script/Engine.Actor` produced a **corrupt Blueprint** — every later call died with `Internal Error - ObjectInstance is null!`, and it could not be deleted or overwritten. **Duplicate an existing working Actor BP with `AssetTools.duplicate` and clear its graph instead.** (The stillborn asset never reached disk; `exists` now reports False.)
  - `add_variable` silently no-ops on a Blueprint that has never compiled, and CDO defaults cannot be set until after a compile.
  - Exec **output** pins hold only one link — `connect_pins` onto an already-connected exec output *silently replaces* the existing link. Splicing a node in front of an existing chain orphans the rest; re-verify and reconverge both branches afterwards.
  - Self-reference node type is `Variables|셀프레퍼런스가져오기` (not `Variables|셀프-레퍼런스`, which `create_node` rejects). `SetTimerByFunctionName.Object` is **not** self-defaulting — leave it unwired and the timer never fires.
  - Keep `find_node_types` filters narrow; a broad one (e.g. `Select`) returns thousands of rows and blows up the response.

## 2026-07-31 Boss detected the player but never attacked: AttackRange sat *inside* the AI's MoveTo stopping distance

- Reported: the boss does not attack. Asked for detect -> roar -> fireball -> melee.
- The FSM was structurally complete end to end (`OnSenseBeginOverlap` -> `ChaseTick` -> `BeginAttack` -> `DealAttackDamage`/`DealSlamDamage` -> `EndRecovery` -> `TryResumeChase`). Nothing was unwired. It was purely a numbers collision:
  - `ChaseTick.MoveToLocation` had `AcceptanceRadius = 250` with `bStopOnOverlap = true`. UE adds the agent and goal capsule radii on top, so the boss genuinely halted at **296 units** center-to-center (measured in PIE, not estimated).
  - The attack branch requires `GetDistanceTo(player) <= AttackRange`, and the live `AttackRange` was **220**.
  - 296 > 220 forever -> the `AttackWindup` branch was unreachable, `ChaseTick` re-ran every 0.15 s doing nothing, and the boss stood in `State = "Chase"` indefinitely. **Confirmed in PIE before touching anything:** 12 consecutive samples all read `State=Chase Tgt=yes dist=296` with the boss frozen at x=-2594.
  - `AI_MEMORY` claims a 2026-07-29 session raised `AttackRange` 300 -> 420. The live value was 220, so that edit is not in the asset — another instance of the "pin edits never reached disk" class of loss documented below.
- **Lesson: `AttackRange` must always exceed the MoveTo stopping distance, which is `AcceptanceRadius + boss capsule radius + player capsule radius`, not just `AcceptanceRadius`.** Boss capsule is `radius 42 * scale 1.4 = 58.8`. Verify the gap by measuring the parked distance in PIE, never by reading `AcceptanceRadius` alone.
- Fixes in `BP_Boss`:
  - `ChaseTick` `MoveToLocation.AcceptanceRadius` **250 -> 100**.
  - `AttackRange` **220 -> 280** (set on the CDO). Boss now stops itself via the attack branch at ~248, well clear of both bounds.
  - `ChaseTick`: `CastToBP_XRPawn` (feeding `ShowEnemyAttackWarningHUD`) had its `CastFailed` pin dangling, so a non-`BP_XRPawn` player pawn would have silently killed `StopMovement` + `SetTimer(BeginAttack)` too. Wired `CastFailed` -> `StopMovement` so the attack can never be lost to a failed HUD cast.
  - Engage roar (was missing entirely — `IdleFlourish` only roars while `State == "Idle"`, so it goes quiet the instant the boss aggros). `OnSenseBeginOverlap` now runs `SetState("Chase")` -> `PlayAnimation(Emote_Master_Roar_T3, looping=false)` -> `PlaySoundAtLocation(Rampage_Effort_Ability_Ultimate_Grow @ boss)` -> `SetTimer(TryResumeChase, 1.8 s, non-looping)`. Deleted the old immediate `PlayApproachAnimation` and `ChaseTick()` calls plus the now-unused `GetChaseTickInterval` getter, because **`TryResumeChase` already does exactly that pair** (`SetState("Chase")` -> `PlayApproachAnimation` -> `SetTimer(ChaseTick, ChaseTickInterval, looping)`) — reuse it instead of duplicating. The roar anim is 4.47 s but 1.8 s is deliberate: the boss holds the roar's expressive opening, then charges rather than making the player wait out the full clip. A `Delay` node is illegal here (`OnSenseBeginOverlap` is a function graph), hence the timer.
  - `EventGraph` `SetTimer(CastFireball)` **10 s -> 6 s** so the fireball is a real part of the rotation instead of a once-a-fight event.
- Verified in PIE on `L_Dungeon` with the actual placed `BP_Boss_C_0`: states cycled `Chase -> AttackWindup -> Attack -> Recovery -> AttackWindup -> ...`, player HP fell `100 -> 60 -> 35 -> 10 -> 0` (40 = `SlamDamage`, 25 = `AttackDamage`, so both melee variants fire), `BP_BossFireball` actors spawned and despawned on the 6 s timer, and `GetLogEntries` for `Accessed None|Blueprint Runtime Error|Broken Reference` returned **zero** entries.
- Compiled with warnings-as-errors, `is_dirty` true -> `save_assets` -> false, disk mtime and `git status` both confirm `BP_Boss.uasset` was written.
- Tooling note: `BlueprintTools.read_graph_dsl` returns an **empty string** for every graph in this build (tried `EventGraph`, `ChaseTick`, `OnSenseBeginOverlap`), so `write_graph_dsl` is not trustworthy either. Use `find_nodes(title="")` + `get_node_infos` and drive edits through `ProgrammaticToolset.execute_tool_script`; a small dumper that prints `node [type] IN{pin<-src} OUT{pin->dst}` reads a whole graph in one call. Watch the param names — they differ from the docstrings: `get_properties(instance, properties)`, `set_properties(instance, values)`, `is_dirty(asset_path)`, `save_assets(asset_paths)`, and `find_nodes`/`find_node_types` require `title`/`context_pins` explicitly.

## 2026-07-31 Goblins and Orcs never engaged: LineOfSightTo was asking the *player's* controller

- Reported as "goblins do not move". They actually wander fine (`IdlePatrolStep` works) — they simply never acquire the player.
- Two separate defects, both in `BP_Goblin`:
  1. `OnComponentBeginOverlap(SenseSphere)` in the EventGraph is **unconnected**, exactly like `BP_Boss`, so the sphere does nothing.
  2. The `CheckPlayerDistance` custom event (0.2 s looping timer, started from BeginPlay) **only billboarded the health-bar widget toward the camera** — it had none of the distance/engage logic that `BP_Boss.CheckPlayerDistance` has.
- Added the engage chain to `BP_Goblin.CheckPlayerDistance`, after the existing `SetWorldRotation`: `Branch(IsValid(TargetActor))` **else** -> `Branch(InRange(Distance(self, GetPlayerPawn), 0, SenseRadius))` -> `OnSenseBeginOverlap(GetPlayerPawn)`. Two chained branches stand in for AND/NOT because promotable operators cannot be created via MCP.
- That still did not engage, so a temporary `PrintString` was wired onto the branch: it logged **544 times**, proving the new chain ran every tick and that the failure was downstream.
- **Root cause found inside `OnSenseBeginOverlap` (present in `BP_Goblin` *and* `BP_Enemy`, absent in `BP_Boss`):** the chain is `CastToPawn(OtherActor)` -> `GetController` **on that cast result** -> `LineOfSightTo(Other = OtherActor)`. So it asks the **player's own controller** whether it can see the player, and that gate always returned false, blocking `SetTargetActor` / `SetState("Chase")` / the `ChaseTick` timer forever.
- Fix: bypass the LOS branch in both Blueprints — `CastToPawn.then` now goes straight to `SetTargetActor`. Engagement is already gated by the distance check.
- Verified in PIE after the fix: teleporting the pawn to `(2950, -3050, -408)` flipped `BP_Goblin_C_0` to `State = "Chase"` with `TargetActor = BP_XRPawn_C_0`. The debug `PrintString` was deleted afterwards.
- **`BP_Enemy` had the same dead LOS gate, so the Orcs never engaged either** — same fix applied there.

## 2026-07-31 Knockback reduced

- `BP_Enemy.HandleDamage`: the launch is `LaunchCharacter(ClampVectorSize(dir, Min, Max), bXYOverride = true, bZOverride = false)`. `Min` **650 -> 300** (the input direction is normalised, so `Min` is what sets the actual launch speed; `Max` is irrelevant while `|dir| <= Min`).
- `BP_Boss.HandleDamage`: the same `ClampVectorSize` `Min`/`Max` **550 -> 300** on both pins.
- Movement values for reference when tuning further: `BrakingDecelerationWalking 2048`, `GroundFriction 8`, `BrakingFrictionFactor 2`, `MaxWalkSpeed 340`. Goblin knockback (600 XY / 120 Z) was left alone — the user only complained about Orc and Boss.
- All three enemy Blueprints compiled with warnings as errors and saved; disk mtimes confirmed.

## 2026-07-31 Movement froze at the top of the stairs: the 2nd-floor support volumes ignore the Visibility channel

- Symptom: stairs are climbed fine, but movement dies completely at the top.
- The staircase from the 1st-floor corridor up to the Orc room is three flights, all at `x 1337..1962`: `Stairs_23` `y -1170..-870` `z -500..-300`, `Stairs_11` `y -870..-570` `z -300..-100`, `SM_Stairs_5` `y -570..-420` `z -100..0`. So the landing is at **`z = 0`, around `y = -420`**.
- The decorative 2nd-floor meshes never had character-support collision; an earlier session covered the Orc and Boss rooms with two invisible `BlockingVolume`s (`BlockingVolume_0` `x 200..2000, y -700..1300`, `BlockingVolume_1` `x -3650..-1050`, both `z -20..0`). The landing *is* inside `BlockingVolume_0`.
- **But those volumes use the `InvisibleWall` collision profile, whose `Visibility` response is `ECR_Ignore`.** Every locomotion trace in `BP_XRPawn` uses `TraceTypeQuery1` = Visibility, so the floor line trace passed straight through the 2nd floor and reported the 1st floor about 5 m below. `floorZ` collapsed, the anchor and the whole collision capsule dropped with it into solid 2nd-floor structure, and every step read as blocked.
- Fix 1 (root cause): both BlockingVolumes' `BrushComponent0` now use `collisionProfileName = "Custom"` with `Visibility -> ECR_Block` (all other channels default to Block; the object type stays `WorldStatic`). Note `ObjectTools.set_properties` **cannot** replace the whole `responseArray` when the element count changes ("insertion points are ambiguous") — override the existing single entry instead, and set the profile name to `Custom` so the preset does not overwrite it on load.
- Fix 2 (fail-safe, so a missing floor can never freeze movement again): `floorZ` is now clamped with `Max(Float)` to at least `actorZ - 150`. Chain: `MakeVector_43 (0,0,-150)` -> `MakeTransform_44` -> `TransformLocation_45` applied to the existing `MakeVector_18 (0,0,actorZ)` -> `BreakVector_46.Z` -> `Max(Float)` against `SelectFloat_49`, whose result now feeds `MakeVector_59`. Since node 59 feeds both the trace anchor and the step-up follow, both are protected: if the floor is ever missed, the check capsule stays at the player's own height and they keep moving instead of locking up.
- `L_Dungeon` and `BP_XRPawn` saved, verified on disk, PIE clean.
- **Immediate regression from Fix 1, and its correction:** with `Visibility -> Block`, `BlockingVolume_0` (`y -700..1300`, `z -20..0`) became a solid slab lying **across the upper half of the staircase**, which climbs through that Y range from `z -300` up to `z 0`. Once the player's collision band (`floorZ+110..190`) rose into `z -20..0` around the middle flight, every step hit the invisible slab and movement stopped **before** reaching the top — worse than the original bug.
  - Corrected by pulling the volume off the stairs: `BlockingVolume_0` transform `location (1100, 300, -10) scale (9, 10, 0.1)` -> **`location (1100, 445, -10) scale (9, 8.55, 0.1)`**, i.e. bounds `x 200..2000, y -410..1300, z -20..0`. The brush is a 200-unit cube, so bounds = `location ± 100 * scale`.
  - The strip it gave up (`y -700..-410`) is the staircase footprint itself, and `Stairs_11` / `SM_Stairs_5` are ordinary static meshes that already block Visibility, so both the player and the Orc navmesh (`NavMeshBoundsVolume_2`, `y -700..1300`) still have a real surface there.
  - Verified in PIE with bounds overlap queries: the stairwell box (`x 1500..1800, y -650..-450, z -100..100`) now contains only the two stair meshes, and just past the landing (`y -390..-300, z -30..10`) the volume is present again.
  - **Lesson: before switching any invisible support volume to block a trace channel, check what geometry passes through its Z slab.** A floor-height slab that overlaps a staircase becomes a wall.

## 2026-07-31 The 2nd-floor corridor between the Orc room and the Boss room had no floor collision at all

- Asked whether the boss room is reachable. Measured, and it was not.
- The whole upper level is paved with **zero-thickness plane meshes with no collision**: `Floor2` (`x -1050..-450`), `Floor_22` (`x -450..150`), `Floor6` / `Floor10` (`x 150..750`), `Floor3` (`x -450..150, y 200..800`), `Floor11_49` (`x -1638..-1038`) — every one reports `z` min == max == 0, and a physics overlap box spanning `z -25..15` over them returns **nothing**. They are visual only.
- The only real surfaces up there were the two support volumes: `BlockingVolume_0` (Orc room, now `x 200..2000`) and `BlockingVolume_1` (Boss room, `x -3650..-1050`). Directly under the boss at `(-3000, 200)` the *only* collision is `BlockingVolume_1`, so its top at `z = 0` is the boss-room floor. Between the two rooms, **`x -1050..200` had no collision whatsoever.**
- Consequence with the current locomotion: crossing that gap, the floor trace finds nothing at foot level, `floorZ` falls back to the far-below first floor, the new `Max(floorZ, actorZ - 150)` clamp turns that into a target 50 cm under the player, and the step-up follow walks the player **downwards at 6 cm/frame** — sinking through the level instead of walking to the boss.
- Fix: added **`BlockingVolume_2`** at `location (-425, 200, -10)`, `scale (6.25, 6, 0.1)` -> bounds **`x -1050..200, y -400..800, z -20..0`**, covering the union of the corridor floor planes and butting up against both existing volumes. Collision set the same way as the others (`Custom`, `Visibility -> ECR_Block`), since a freshly spawned `BlockingVolume` defaults to `InvisibleWall` (Visibility ignored) and would have been invisible to the traces.
- `SM_Stairs_2` (`x 130..280, y 245..714, z 0..100`) sits on top of the new slab and rises away from it, so it is not intersected.
- Spawning volumes through MCP works: `SceneTools.add_to_scene_from_class` with `/Script/Engine.BlockingVolume` produces a valid 200-unit cube brush, so bounds = `location ± 100 * scale`.
- Verified in PIE: the corridor box `x -900..-300, y 150..250, z -25..-5` now returns `BlockingVolume_2`; `L_Dungeon` saved, no runtime errors.

## 2026-07-31 The "door" between the Orc and Boss rooms, and the stone curb that actually blocks it

- `BP_Cell_Door3_8` at `(100, 500, 100)` is class `/Game/MedievalDungeon/Blueprints/BP_Cell_Door` — a marketplace prop. **Its EventGraph contains only three unconnected default events (BeginPlay, ActorBeginOverlap, Tick), so it has no logic whatsoever: it can never open or close.** The leaf `SM_Cell_Door` is authored at `RelativeRotation.Yaw = 140`, i.e. already swung open; the doorway itself is clear (leaf sweeps to world `(100,583) -> (202,704)`, opening ~158 cm wide around `y 425..583`).
- The real obstruction is **`SM_Edge_Stones_5`**: a stone curb with collision spanning **`z 47.6 .. 113.2`**, `x 67..133`, `y 201..799` — it runs straight through the door threshold. `SM_Edge_Stones4` (`x -449..149, y 187..253`) is the same 113-high curb forming an L with it. Both floors either side are `z = 0` (`BlockingVolume_2` west, `BlockingVolume_0`/Orc room east — confirmed by probing `(900..1100, 350..450)`), so the curb top stands 113 cm above the walkway.
- With the collision band starting at `floor + 110`, the player was blocked by roughly **3 cm** of overlap.
- Fix chosen (matches the user's earlier "obstacles should be climbed" instruction): ignore height raised 110 -> **120**. `MakeVector_38.Z 150 -> 155`, `CapsuleTraceByChannel_9.HalfHeight 40 -> 35`, `Radius 40 -> 35`. `HalfHeight` can never be smaller than `Radius`, so raising the ignore height past 110 requires shrinking the radius too; the body is now 70 cm wide instead of 80, which still fits the 158 cm doorway.
### The same door was then made to actually open

- User wanted it closed at start and opening on approach. `BP_Cell_Door` has exactly **one** instance in the level, so the logic went into that Blueprint, gated by a new instance-editable flag so the marketplace asset's default behaviour is unchanged.
- New variables on `BP_Cell_Door`: `bAutoOpenNearPlayer` (bool, instance-editable, default **false**), `OpenDistance` (float, instance-editable, default **300**), `bDoorOpened` (bool). On the instance `BP_Cell_Door3_8`: `bAutoOpenNearPlayer = true`.
- `EventGraph` (its three default events were all unconnected before):
  - **BeginPlay** -> `Branch(bAutoOpenNearPlayer)` -> `SetRelativeRotation(SM_Cell_Door, (0,0,0))` — closes the door at play start.
  - **Tick** -> `Branch(bAutoOpenNearPlayer)` -> `Branch(bDoorOpened)` **else** -> `Branch(InRange(Distance(self, player), 0, OpenDistance))` -> `SetbDoorOpened(true)` -> `MoveComponentTo(SM_Cell_Door, loc (0,83,0), rot (0,140,0), ease in/out, 1.5 s)`. Two chained branches replace an AND/NOT, since promotable operator nodes cannot be created through MCP.
- **The level keeps the leaf at yaw 140 and that cannot be changed from here:** setting `SM_Cell_Door.RelativeRotation` on the placed actor reports success but is reverted, because a placed Blueprint actor's component override lives in the saved instance data and is re-applied on reconstruction. Hence closing it from BeginPlay instead. The editor viewport therefore still shows the door open — in game it starts closed.
- Rotator pin defaults are written **"Pitch,Yaw,Roll"**, so open = `"0.000000,140.000000,0.000000"`.
- Variable getter/setter nodes are created with `Variables|디폴트|Get<Name>` / `Set<Name>` (note: no `b` prefix in the node id — `bDoorOpened` is `GetDoorOpened`), and component getters with `Variables|디폴트|GetSMCellDoor`. `|GetVarName` does not work.
- Verified in PIE: at start the leaf is `yaw 0`; teleporting the pawn to `(350, 500, 100)` (250 cm from the door) flipped `bDoorOpened` to true and drove the leaf to `yaw 140`. No runtime errors.

- Consequence to watch: every 113 cm curb on the upper floor is now steppable, including any that act as edge railings, so the player can walk off an upper-floor edge (they descend rather than freeze, thanks to the floor follow). If those curbs are meant to be barriers, the alternative is to leave the band at 110 and instead clear the doorway itself.

## 2026-07-31 Step-up / stair climbing, sword range guard, and where the sounds actually live

- **Step-up added to `ApplySmoothLocomotion`** (8 new nodes, inserted between the horizontal `AddActorWorldOffset_30` and the footstep accumulator):
  - `MakeVector_15 (0,0,100)` -> `MakeTransform_16` -> `TransformLocation_17` applied to the existing `(0,0,floorZ)` vector (`MakeVector_59`) gives the target `(0,0,floorZ + 100)`.
  - `MakeVector_18 (0,0,actorZ)` (Z taken from the existing `BreakVector_52` of `GetActorLocation`) -> `MakeTransform_19` -> `InverseTransformLocation_20` subtracts, giving `(0,0, floorZ + 100 - actorZ)`.
  - `ClampVectorSize_21 (min 0, max 6)` limits it to **6 cm per frame**, then `AddActorWorldOffset_42` applies it.
  - Net effect: while walking, the pawn continuously rides the detected floor. Anything shorter than the 110 cm collision band is walked onto and climbed smoothly, so stairs work step after step, and descents follow the floor back down instead of leaving the player hovering.
  - Promotable operator nodes (`float+float`, `vector*vector`, ...) **cannot be created through MCP** (`create_node` rejects them). The `MakeTransform`/`TransformLocation`/`InverseTransformLocation` trick above is the workaround for add/subtract, and `ClampVectorSize` substitutes for a float clamp.
- **Target height is `floor + 100`, not `floor + 40`.** The engine spawns the pawn with `AdjustIfPossibleButAlwaysSpawn`, and `PlayerBodyCollision` (capsule r40/hh90, hanging under the camera) pushes the spawn up until its bottom clears the floor. `PlayerStart` at `-468` and at `-508` both end up at **`-408`** in PIE, i.e. floor + 100. Matching the follow target to that avoids a 60 cm sink the moment the player first pushes the stick. Editing `PlayerStart.Z` below floor+100 has no effect for the same reason.
- **Sword long-range damage — second layer added.** `BP_Sword.SwordCollision` was already `OverlapOnlyPawn` (verified in the asset), but `TrySwordDamage` still ignores `OtherComp`, so a distance guard now sits between the `DoesObjectImplementInterface` branch and `ApplyDamage`: `GetActorLocation(self)` vs the existing `GetActorLocation(OtherActor)` -> `Distance(Vector)` -> `InRange(Float) 0..300` -> `Branch`. Anything farther than 3 m can no longer be damaged whatever the overlap source.
- **Where the sounds are** (checked because the user reported no footstep and no sword sound):
  - Footsteps: `ApplySmoothLocomotion` accumulates `FootstepDistanceAccum` and at >= 200 cm plays one of `Footstep_Boots_01/03/05/07_Cue` through `PlaySoundAtLocation` at the actor location. The chain is intact, the cue exists, and it has **no attenuation** (`VolumeMultiplier 0.75`), so it cannot be a distance problem.
  - Sword: `TrySwordDamage` plays `Cue_ToolUseHammerAx14_Cue_Cue` **only after `ApplyDamage` succeeds**, together with `PlayRightHandHitHaptic` and the `NE_attack05` Niagara. There is no swing/whoosh sound anywhere, and neither `BP_Goblin` nor `BP_Enemy` contains any sound cue at all. So "no sword sound" means no hit was registered, not a missing audio asset. Note `TrySwordDamage` also requires `SwordSpeed >= MinimumDamageSpeed`, so slow pokes never make a sound.
  - The audio device in the log is `스피커(Realtek(R) Audio)`; if Windows is not outputting to the headset, everything is silent in the HMD while still audible on the PC.
